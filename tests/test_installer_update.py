...
from zotbridge.installer_update import build_update_plan, detect_install_method, run_update


class InstallerUpdateTests(unittest.TestCase):
    def test_detects_uv_tool_installation(self):
        method = detect_install_method(
            prefix="/Users/example/.local/share/uv/tools/zotbridge",
            executable="/Users/example/.local/share/uv/tools/zotbridge/bin/python",
            argv0="/Users/example/.local/bin/zotbridge",
        )
        self.assertEqual(method, "uv-tool")

    def test_detects_pipx_installation(self):
        method = detect_install_method(
            prefix="/Users/example/.local/pipx/venvs/zotbridge",
            executable="/Users/example/.local/pipx/venvs/zotbridge/bin/python",
            argv0="/Users/example/.local/bin/zotbridge",
        )
    ...
    def test_build_update_plan_for_uv_tool(self):
        ...
        self.assertEqual(plan.command, ["/usr/local/bin/uv", "tool", "upgrade", "zotbridge"])
    ...
    def test_build_update_plan_for_pipx(self):
        ...
        self.assertTrue(plan.auto_supported)
        self.assertEqual(plan.command, ["/usr/local/bin/pipx", "upgrade", "zotbridge"])

    def test_build_update_plan_for_virtualenv(self):
        plan = build_update_plan(
            prefix="/tmp/venv",
            executable="/tmp/venv/bin/python",
            argv0="/tmp/venv/bin/zotbridge",
        ...
        self.assertTrue(plan.auto_supported)
        self.assertEqual(plan.command, ["/tmp/venv/bin/python", "-m", "pip", "install", "--upgrade", "zotbridge"])

    def test_build_update_plan_for_unknown_install(self):
        plan = build_update_plan(
            prefix="/usr/local",
            executable="/usr/local/bin/python3",
            argv0="/usr/local/bin/zotbridge",
            base_prefix="/usr/local",
            virtual_env="",
            uv_path=None,
            pipx_path=None,
    ...
    @patch("zotbridge.installer_update.subprocess.run")
    @patch("zotbridge.installer_update.current_version")
