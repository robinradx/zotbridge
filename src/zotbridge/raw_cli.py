...
def _add_machine_commands(sub) -> None:
    ...
    qmd_embed = qmd_sub.add_parser("embed")
    qmd_embed.add_argument("-f", "--force", action="store_true")
    qmd_embed.add_argument("--max-docs-per-batch", type=int)
    qmd_embed.add_argument("--max-batch-mb", type=int)
    qmd_get = qmd_sub.add_parser("get")
    qmd_get.add_argument("target")
    qmd_get.add_argument("--from", dest="line_from", type=int)
    qmd_get.add_argument("--count", dest="line_count", type=int)
    qmd_sub.add_parser("doctor")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="zotbridge",
        description="zotbridge CLI for Web API sync, HTTP/MCP access, semantic search, and recovery.",
    )
    ...
    setup_show.add_argument("tool", choices=_target_choices(*[tool for tool in SUPPORTED_SETUP_TARGETS if tool != "all"]))
...
def main(argv: list[str] | None = None) -> int:
    ...
    if command == "setup":
        ...
        if args.setup_command in {"start", "wizard", "account", "libraries"}:
            mode = {
                "start": "full",
                "wizard": "full",
                "account": "account",
        ...
        if args.setup_command == "add":
            payload = install_mcp_setup(args.tool, settings, cwd=cwd, scope=args.scope)
            _emit(
                payload if args.tool != "all" else payload["results"],
                as_json=args.json,
                renderer=(
                    lambda payload: _render_install_results(payload, heading="Setup applied")
                    if args.tool == "all"
                    else render_install_result(payload, heading="Setup applied")
                ),
            )
            return 0
        if args.setup_command == "remove":
            payload = remove_mcp_setup(args.tool, cwd=cwd, scope=args.scope)
            _emit(
                payload if args.tool != "all" else payload["results"],
                as_json=args.json,
                renderer=(
                    lambda payload: _render_install_results(payload, heading="Setup removed")
                    if args.tool == "all"
                    else render_install_result(payload, heading="Setup removed")
                ),
            )
            return 0
    ...
    if command == "qmd":
        ...
        if args.qmd_command == "embed":
            ...
            return 0
        if args.qmd_command == "get":
            _print(client.get(args.target, line_from=args.line_from, line_count=args.line_count))
            return 0
        if args.qmd_command == "doctor":
