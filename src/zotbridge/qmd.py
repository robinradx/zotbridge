...
class QmdClient:
    ...
    @staticmethod
    def _unsupported_format_option(result: subprocess.CompletedProcess[str]) -> bool:
        text = f"{result.stdout}\n{result.stderr}".lower()
        return "--format" in text and any(token in text for token in ("unknown", "unrecognized", "invalid option"))
    ...
    def doctor(self) -> dict[str, Any]:
        ...
        payload["version"] = version.stdout.strip() or version.stderr.strip()
        payload["version_ok"] = version.returncode == 0

        doctor = self._run(["doctor"])
        unsupported = "unknown command" in f"{doctor.stdout}\n{doctor.stderr}".lower()
        payload["doctor"] = {
    ...
    def _write_markdown(
        ...
        escaped_title = (title or "").replace('"', '\\"')
        lines = [
            "---",
    ...
    def embed(
        ...
        *,
        ...
        if max_docs_per_batch is not None:
            args.extend(["--max-docs-per-batch", str(max_docs_per_batch)])
        if max_batch_mb is not None:
            args.extend(["--max-batch-mb", str(max_batch_mb)])
    ...
    def search(self, mode: str, query: str, *, limit: int = 10, library_id: str | None = None) -> Any:
        self.ensure_collection()
        args = [mode, query, "--format", "json", "-n", str(limit), "-c", self.settings.qmd_collection]
        result = self._run(args)
        if result.returncode != 0 and self._unsupported_format_option(result):
        ...
        payload = self._json_or_raw(result.stdout)
        if library_id:
            filtered = []
    ...
    def get(self, target: str, *, line_from: int | None = None, line_count: int | None = None) -> Any:
        if line_count is not None and line_from is None:
            raise ValueError("line_from is required when line_count is set")
        qmd_target = target
        if line_from is not None:
            qmd_target = f"{qmd_target}:{line_from}"
            if line_count is not None:
                qmd_target = f"{qmd_target}:{line_count}"
        args = ["get", qmd_target, "--format", "json"]
        result = self._run(args)
        if result.returncode != 0 and self._unsupported_format_option(result):
            result = self._run(["get", qmd_target])
        if result.returncode != 0:
            raise RuntimeError(result.stderr.strip() or result.stdout.strip() or "qmd get failed")
        return self._json_or_raw(result.stdout)
