...
class Settings:
    ...
    def resolved_mirror_db(self) -> Path:
        return Path(self.mirror_db).expanduser() if self.mirror_db else self.resolved_state_dir() / "mirror.sqlite"
...
def default_profile_name() -> str | None:
    return normalize_profile_name(os.environ.get("ZOTBRIDGE_PROFILE"))
...
def _settings_from_payload(payload: dict[str, Any], *, selected_profile: str | None) -> Settings:
    ...
    cleaned = {
        ...
        if key in valid_fields and key != "selected_profile"
    }
    return Settings(**cleaned, selected_profile=selected_profile)
...
def load_settings(path: Path | None = None, *, profile: str | None = None, ensure_dirs: bool = True) -> Settings:
    ...
    env_api_key = os.environ.get("ZOTBRIDGE_API_KEY")
    env_user_id = os.environ.get("ZOTBRIDGE_USER_ID")
    env_api_base = os.environ.get("ZOTBRIDGE_API_BASE")
    env_state_dir = os.environ.get("ZOTBRIDGE_STATE_DIR")
    env_daemon_host = os.environ.get("ZOTBRIDGE_DAEMON_HOST")
    env_daemon_port = os.environ.get("ZOTBRIDGE_DAEMON_PORT")
    ...
    if env_state_dir:
        settings.state_dir = env_state_dir
    if env_daemon_host:
