...
def build_metrics_text(settings: Settings) -> str:
    ...
    lines = [
        "# HELP zotbridge_runtime_running Whether the zotbridge daemon runtime is running.",
        "# TYPE zotbridge_runtime_running gauge",
        f"zotbridge_runtime_running {1 if runtime else 0}",
        "# HELP zotbridge_http_requests_total Total HTTP requests served by the daemon runtime.",
        "# TYPE zotbridge_http_requests_total counter",
        f"zotbridge_http_requests_total {int(runtime.get('request_count') or 0)}",
        "# HELP zotbridge_background_sync_enabled Whether periodic background sync is enabled.",
        "# TYPE zotbridge_background_sync_enabled gauge",
        f"zotbridge_background_sync_enabled {1 if background.get('enabled') else 0}",
        "# HELP zotbridge_background_sync_running Whether a background sync job is currently running.",
        "# TYPE zotbridge_background_sync_running gauge",
        f"zotbridge_background_sync_running {1 if background.get('running') else 0}",
        "# HELP zotbridge_background_sync_runs_total Total completed background sync runs.",
        "# TYPE zotbridge_background_sync_runs_total counter",
        f"zotbridge_background_sync_runs_total {int(background.get('runs_total') or 0)}",
        "# HELP zotbridge_background_sync_successes_total Total successful background sync runs.",
        "# TYPE zotbridge_background_sync_successes_total counter",
        f"zotbridge_background_sync_successes_total {int(background.get('successes_total') or 0)}",
        "# HELP zotbridge_background_sync_failures_total Total failed background sync runs.",
        "# TYPE zotbridge_background_sync_failures_total counter",
        f"zotbridge_background_sync_failures_total {int(background.get('failures_total') or 0)}",
        "# HELP zotbridge_background_sync_last_success_unixtime Unix timestamp of the last successful background sync.",
        "# TYPE zotbridge_background_sync_last_success_unixtime gauge",
        f"zotbridge_background_sync_last_success_unixtime {_iso_to_unix(background.get('last_success_at'))}",
    ]
    return "\n".join(lines) + "\n"
