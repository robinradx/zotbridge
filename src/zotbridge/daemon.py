...
def current_daemon_status(settings: Settings | None = None) -> DaemonStatus:
    ...
    runtime_message = "The zotbridge daemon is implemented and can host the HTTP API and core services directly."
    if runtime_running:
        runtime_message = (
            f"zotbridge-daemon is running at http://{settings.daemon_host}:{settings.daemon_port} "
            "and serving the zotbridge core API."
        )
    runtime_mode = "running" if runtime_running else "implemented"
    message = (
        "The zotbridge daemon runtime is available."
        if not runtime_running
        else "The zotbridge daemon runtime is running."
    )
    return DaemonStatus(
        available=runtime_running,
        mode="zotbridge-runtime-ready",
        message=message,
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="zotbridge-daemon")
