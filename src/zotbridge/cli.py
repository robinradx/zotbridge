...
from .daemon import build_runtime_command, current_daemon_status, serve_daemon_runtime
...
app = typer.Typer(
    ...
    help="Human-friendly zotbridge CLI. Use [bold]zotbridge raw ...[/bold] for strict machine-oriented commands.",
)
setup_app = typer.Typer(no_args_is_help=True, rich_markup_mode="rich", help="Interactive setup and MCP client installation.")
skill_app = typer.Typer(no_args_is_help=True, rich_markup_mode="rich", help="Install or export skills.")
...
qmd_app = typer.Typer(no_args_is_help=True, rich_markup_mode="rich", help="Export Zotero Markdown and query it through qmd.")
...
def _setup_payload(config_path: Path, settings: Settings, *, discovered_libraries=None, selected_remote_libraries=None) -> dict[str, object]:
    warnings: list[str] = []
...
def _runtime_services(settings: Settings):
    ...
    return canonical, store, qmd_indexer, sync_service, service


def _recovery_service(settings: Settings) -> RecoveryService:
    canonical, _, qmd_indexer, _, _ = _runtime_services(settings)
...
def update_command(
    ...
    with console.status(f"Updating zotbridge via {plan.method}...", spinner="dots"):
        payload = run_update(plan)
    if payload.get("updated"):
...
def _qmd_export_payload(settings: Settings, library: str | None = None) -> dict[str, object]:
    canonical, store, _, _, _ = _runtime_services(settings)
    client = QmdClient(settings)
    if library:
        if prefers_canonical_reads(canonical, library):
            return client.export_from_canonical(canonical, library)
        return client.export_from_store(store, library)

    exported = 0
    mirror_exported = 0
    canonical_exported = 0
    for entry in merged_libraries(store, canonical):
        library_id = entry["library_id"]
        if prefers_canonical_reads(canonical, library_id):
            result = client.export_from_canonical(canonical, library_id)
            canonical_exported += int(result["exported"])
        else:
            result = client.export_from_store(store, library_id)
            mirror_exported += int(result["exported"])
        exported += int(result["exported"])
    return {
        "exported": exported,
        "mirror_exported": mirror_exported,
        "canonical_exported": canonical_exported,
        "export_dir": str(settings.resolved_export_dir()),
        "collection": settings.qmd_collection,
    }


@qmd_app.command("export")
def qmd_export_command(
    ctx: typer.Context,
    library: str | None = typer.Option(None, "--library", help="Export one library, such as user:123456."),
) -> None:
    _emit(ctx, _qmd_export_payload(_human_settings(), library), title="qmd export")


@qmd_app.command("embed")
def qmd_embed_command(
    ctx: typer.Context,
    force: bool = typer.Option(False, "-f", "--force", help="Refresh embeddings even when qmd thinks they are current."),
    max_docs_per_batch: int | None = typer.Option(None, "--max-docs-per-batch", help="Cap documents loaded per embedding batch."),
    max_batch_mb: int | None = typer.Option(None, "--max-batch-mb", help="Cap UTF-8 MB loaded per embedding batch."),
) -> None:
    payload = {
        "stdout": QmdClient(_human_settings()).embed(
            force=force,
            max_docs_per_batch=max_docs_per_batch,
            max_batch_mb=max_batch_mb,
        )
    }
    _emit(ctx, payload, title="qmd embed")


@qmd_app.command("query")
def qmd_query_command(
    ctx: typer.Context,
    query: str = typer.Argument(..., help="Hybrid semantic query."),
    library: str | None = typer.Option(None, "--library", help="Limit results to one library."),
    limit: int = typer.Option(10, "-n", "--limit", help="Maximum number of results."),
) -> None:
    _emit(ctx, QmdClient(_human_settings()).search("query", query, limit=limit, library_id=library), title="qmd query")


@qmd_app.command("search")
def qmd_search_command(
    ...
    _emit(ctx, QmdClient(_human_settings()).search("search", query, limit=limit, library_id=library), title="qmd search")


@qmd_app.command("vsearch")
def qmd_vsearch_command(
    ctx: typer.Context,
    query: str = typer.Argument(..., help="Vector search query."),
    library: str | None = typer.Option(None, "--library", help="Limit results to one library."),
    limit: int = typer.Option(10, "-n", "--limit", help="Maximum number of results."),
) -> None:
    _emit(ctx, QmdClient(_human_settings()).search("vsearch", query, limit=limit, library_id=library), title="qmd vsearch")


@qmd_app.command("get")
def qmd_get_command(
    ctx: typer.Context,
    target: str = typer.Argument(..., help="qmd hit target, doc id, qmd URI, or exported Markdown path."),
    line_from: int | None = typer.Option(None, "--from", help="Start line for qmd range recovery."),
    line_count: int | None = typer.Option(None, "--count", help="Number of lines to recover."),
) -> None:
    _emit(ctx, QmdClient(_human_settings()).get(target, line_from=line_from, line_count=line_count), title="qmd get")


@qmd_app.command("doctor")
def qmd_doctor_command(ctx: typer.Context) -> None:
    _emit(ctx, QmdClient(_human_settings(ensure_dirs=False)).doctor(), title="qmd doctor")
...
def setup_show_command(
    ...
    if tool == "all":
        raise typer.BadParameter("`all` is only supported for setup add/remove.")
...
def setup_add_command(
    ...
    if tool == "all":
        _emit(
            ctx,
            payload["results"],
            renderer=lambda entries: _render_install_results(entries, heading="Setup applied"),
            title="Setup",
        )
        return
...
def setup_remove_command(
    ...
    if tool == "all":
        _emit(
            ctx,
            payload["results"],
            renderer=lambda entries: _render_install_results(entries, heading="Setup removed"),
            title="Setup",
        )
        return
...
def skill_export_command(
    ...
    if tool == "claude-desktop":
        _emit(ctx, payload, renderer=lambda entry: render_install_result_rich(entry, heading="Skill exported"), title="Skill export")
        return
...
def daemon_command_command(ctx: typer.Context) -> None:
    ...
    payload = {
        ...
        "api_url": f"http://{settings.daemon_host}:{settings.daemon_port}",
    }
...
def citations_enable_command(
    ...
    canonical, _, _, _, _ = _runtime_services(settings)
...
def api_command(
    ...
    if ctx.args and ctx.args[0] != "serve":
        raise typer.BadParameter("The human API command only supports `zotbridge api --host ... --port ...`. Use `zotbridge raw api ...` for strict subcommands.")
    settings = _human_settings()
...
def main(argv: list[str] | None = None) -> int:
    prog_name = Path(sys.argv[0]).name if not argv else "zotbridge"
    try:
