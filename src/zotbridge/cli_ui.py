...
def render_version_payload(payload: dict[str, Any]) -> str:
    ...
    return "\n".join(
        [
            f"{payload.get('package', 'zotbridge')} {payload.get('version', 'unknown')}",
...
def render_update_result(payload: dict[str, Any]) -> str:
    ...
    if message:
        ...
        lines.append("Message: zotbridge is already at the installed target version.")
...
def render_doctor_report(report: dict[str, Any]) -> str:
    ...
    qmd_doctor = qmd.get("doctor") or {}
    lines.extend(["", "qmd runtime:"])
    lines.append(f"- Available: {_bool_text(qmd.get('available'))}")
    lines.append(f"- Path: {qmd.get('path') or 'not found'}")
    if qmd.get("version"):
        lines.append(f"- Version: {qmd.get('version')}")
    if qmd_doctor:
        lines.append(f"- Doctor supported: {_bool_text(qmd_doctor.get('supported'))}")
        lines.append(f"- Doctor OK: {_bool_text(qmd_doctor.get('ok'))}")

    lines.extend(["", "Settings:"])
    for key in ("state_dir", "canonical_db", "mirror_db", "export_dir", "file_cache_dir", "recovery_snapshot_dir", "recovery_temp_dir"):
...
def render_version_payload_rich(payload: dict[str, Any]):
    ...
    table.add_row("Package", f"[bold]{payload.get('package', 'zotbridge')}[/bold]")
...
def render_update_result_rich(payload: dict[str, Any]):
    ...
    if message:
        ...
        renderables.append(
            Panel(
                "zotbridge is already at the installed target version.",
                title="Message",
                border_style="yellow",
            )
        )
...
def render_doctor_report_rich(report: dict[str, Any]):
    ...
    qmd_table = _summary_table("qmd runtime")
    qmd_doctor = qmd.get("doctor") or {}
    qmd_table.add_row("Available", _bool_text(qmd.get("available")))
    qmd_table.add_row("Path", str(qmd.get("path") or "not found"))
    if qmd.get("version"):
        qmd_table.add_row("Version", str(qmd.get("version")))
    if qmd_doctor:
        qmd_table.add_row("Doctor supported", _bool_text(qmd_doctor.get("supported")))
        qmd_table.add_row("Doctor OK", _bool_text(qmd_doctor.get("ok")))
    ...
    for key in ("available", "mode", "message", "runtime_running"):
        if key in daemon:
            value = daemon.get(key)
            if isinstance(value, bool):
                value = _bool_text(value)
    ...
    return Group(cli_table, qmd_table, settings_table, daemon_table, setup_table, skills_table)
