...
class RecoveryService:
    ...
    def create_snapshot(self, *, reason: str = "manual") -> dict[str, Any]:
        ...
        if mirror_source.exists():
            mirror_copy = temp_dir / "mirror.sqlite"
    ...
    def execute_restore(
        ...
        *,
        ...
        run = self._create_restore_run(snapshot_id=snapshot_id, library_id=library_id, push_remote=push_remote)
    ...
    def _next_snapshot_id(self) -> str:
        ...
        with tempfile.NamedTemporaryFile(prefix="zotbridge-", suffix=".tmp", dir=self.temp_root(), delete=True) as handle:
